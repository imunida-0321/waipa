...
- **`fireEvent.press` は同一テスト内でも必ず1回ずつ `await act(async () => ...)` で包む**（RNTL v14 の press は Promise を返し、await なしの連打は次の render を overlapping act() で壊す — Task 5 で実証済み）
- **`.value` プロパティ（`playerColor(i).value` 等）を JSX の style 内に直書きしない**（worklets の babel プラグインが SharedValue 誤用と誤認して console.warn を注入する）— 事前に変数へ取り出す
...
export function LivesBar({ names, lives, turnIndex }: Props) {
	return (
		<View style={styles.bar}>
			{names.map((name, i) => {
				// .value を style 内に直書きすると worklets の babel プラグインが
				// SharedValue 誤用として console.warn を注入するため、事前に取り出す
				const color = playerColor(i).value
				return (
					<View
						key={`${i}-${name}`}
						style={[styles.chip, i === turnIndex && { borderColor: color }]}
					>
